import { WETH9, Ether, CurrencyAmount, Percent, Price, sqrt, Token, TradeType } from '@real-wagmi/sdk';
...
describe('Trade', () => {
    ...
    const token3 = new Token(250, '0x0000000000000000000000000000000000000004', 18, 't3', 'token3');

    function v2StylePool(reserve0: CurrencyAmount<Token>, reserve1: CurrencyAmount<Token>, feeAmount: FeeAmount = FeeAmount.MEDIUM) {
    ...
    const pool_weth_0 = v2StylePool(CurrencyAmount.fromRawAmount(WETH9[250], 100000n), CurrencyAmount.fromRawAmount(token0, 100000n));

    const pool_weth_1 = v2StylePool(CurrencyAmount.fromRawAmount(WETH9[250], 100000n), CurrencyAmount.fromRawAmount(token1, 100000n));

    const pool_weth_2 = v2StylePool(CurrencyAmount.fromRawAmount(WETH9[250], 100000n), CurrencyAmount.fromRawAmount(token2, 100000n));

    describe('#fromRoute', () => {
        it('can be constructed with ETHER as input', async () => {
            const trade = await Trade.fromRoute(new Route([pool_weth_0], ETHER, token0), CurrencyAmount.fromRawAmount(Ether.onChain(250), 10000n), TradeType.EXACT_INPUT);
            expect(trade.inputAmount.currency).toEqual(ETHER);
            expect(trade.outputAmount.currency).toEqual(token0);
        });
        it('can be constructed with ETHER as input for exact output', async () => {
            const trade = await Trade.fromRoute(new Route([pool_weth_0], ETHER, token0), CurrencyAmount.fromRawAmount(token0, 10000n), TradeType.EXACT_OUTPUT);
            expect(trade.inputAmount.currency).toEqual(ETHER);
            expect(trade.outputAmount.currency).toEqual(token0);
        });

        it('can be constructed with ETHER as output', async () => {
            const trade = await Trade.fromRoute(new Route([pool_weth_0], token0, ETHER), CurrencyAmount.fromRawAmount(Ether.onChain(250), 10000n), TradeType.EXACT_OUTPUT);
            expect(trade.inputAmount.currency).toEqual(token0);
            expect(trade.outputAmount.currency).toEqual(ETHER);
        });
        it('can be constructed with ETHER as output for exact input', async () => {
            const trade = await Trade.fromRoute(new Route([pool_weth_0], token0, ETHER), CurrencyAmount.fromRawAmount(token0, 10000n), TradeType.EXACT_INPUT);
            expect(trade.inputAmount.currency).toEqual(token0);
            expect(trade.outputAmount.currency).toEqual(ETHER);
        });
    });
    ...
    describe('#fromRoutes', () => {
        it('can be constructed with ETHER as input with multiple routes', async () => {
            const trade = await Trade.fromRoutes<Ether, Token, TradeType.EXACT_INPUT>(
                [
                    {
                        amount: CurrencyAmount.fromRawAmount(Ether.onChain(250), 10000n),
        ...
        it('can be constructed with ETHER as output with multiple routes', async () => {
            const trade = await Trade.fromRoutes<Token, Ether, TradeType.EXACT_OUTPUT>(
                [
                    {
                        amount: CurrencyAmount.fromRawAmount(Ether.onChain(250), 4000n),
                        route: new Route([pool_weth_0], token0, ETHER),
                    },
                    {
                        amount: CurrencyAmount.fromRawAmount(Ether.onChain(250), 6000n),
    ...
    describe('#bestTradeExactIn', () => {
        it('throws with empty pools', async () => {
            await expect(Trade.bestTradeExactIn([], CurrencyAmount.fromRawAmount(token0, 10000n), token2)).rejects.toThrow('POOLS');
        });
        ...
        it('respects maxHops', async () => {
            const result = await Trade.bestTradeExactIn([pool_0_1, pool_0_2, pool_1_2], CurrencyAmount.fromRawAmount(token0, 10n), token2, { maxHops: 1 });
        ...
        it('respects n', async () => {
            const result = await Trade.bestTradeExactIn([pool_0_1, pool_0_2, pool_1_2], CurrencyAmount.fromRawAmount(token0, 10n), token2, { maxNumResults: 1 });

            expect(result).toHaveLength(1);
        });

        it('no path', async () => {
            const result = await Trade.bestTradeExactIn([pool_0_1, pool_0_3, pool_1_3], CurrencyAmount.fromRawAmount(token0, 10n), token2);
            expect(result).toHaveLength(0);
        });

        it('works for ETHER currency input', async () => {
            const result = await Trade.bestTradeExactIn([pool_weth_0, pool_0_1, pool_0_3, pool_1_3], CurrencyAmount.fromRawAmount(Ether.onChain(250), 100n), token3);
            expect(result).toHaveLength(2);
            expect(result[0].inputAmount.currency).toEqual(ETHER);
            expect(result[0].swaps[0].route.tokenPath).toEqual([WETH9[250], token0, token1, token3]);
            expect(result[0].outputAmount.currency).toEqual(token3);
            expect(result[1].inputAmount.currency).toEqual(ETHER);
            expect(result[1].swaps[0].route.tokenPath).toEqual([WETH9[250], token0, token3]);
            expect(result[1].outputAmount.currency).toEqual(token3);
        });

        it('works for ETHER currency output', async () => {
            const result = await Trade.bestTradeExactIn([pool_weth_0, pool_0_1, pool_0_3, pool_1_3], CurrencyAmount.fromRawAmount(token3, 100n), ETHER);
            expect(result).toHaveLength(2);
            expect(result[0].inputAmount.currency).toEqual(token3);
            expect(result[0].swaps[0].route.tokenPath).toEqual([token3, token0, WETH9[250]]);
            expect(result[0].outputAmount.currency).toEqual(ETHER);
            expect(result[1].inputAmount.currency).toEqual(token3);
            expect(result[1].swaps[0].route.tokenPath).toEqual([token3, token1, token0, WETH9[250]]);
    ...
    describe('#maximumAmountIn', () => {
        describe('tradeType = EXACT_INPUT', () => {
            ...
            beforeEach(async () => {
                exactIn = await Trade.fromRoute(new Route([pool_0_1, pool_1_2], token0, token2), CurrencyAmount.fromRawAmount(token0, 100n), TradeType.EXACT_INPUT);
            });
            it('throws if less than 0', () => {
            ...
            it('returns exact if nonzero', () => {
                expect(exactIn.maximumAmountIn(new Percent(0n, 100n)).equalTo(CurrencyAmount.fromRawAmount(token0, 100n))).toBeTruthy();
                expect(exactIn.maximumAmountIn(new Percent(5n, 100n)).equalTo(CurrencyAmount.fromRawAmount(token0, 100n))).toBeTruthy();
                expect(exactIn.maximumAmountIn(new Percent(200n, 100n)).equalTo(CurrencyAmount.fromRawAmount(token0, 100n))).toBeTruthy();
            });
        });
        ...
        describe('tradeType = EXACT_OUTPUT', () => {
            ...
            it('returns slippage amount if nonzero', () => {
                expect(exactOut.maximumAmountIn(new Percent(0n, 100)).equalTo(CurrencyAmount.fromRawAmount(token0, 15488))).toBeTruthy();
                expect(exactOut.maximumAmountIn(new Percent(5n, 100n)).equalTo(CurrencyAmount.fromRawAmount(token0, 16262))).toBeTruthy();
                expect(exactOut.maximumAmountIn(new Percent(200n, 100n)).equalTo(CurrencyAmount.fromRawAmount(token0, 46464))).toBeTruthy();
            });
        });
    });
    ...
    describe('#minimumAmountOut', () => {
        describe('tradeType = EXACT_INPUT', () => {
            let exactIn: Trade<Token, Token, TradeType.EXACT_INPUT>;
            beforeEach(async () => {
                exactIn = await Trade.fromRoute(new Route([pool_0_1, pool_1_2], token0, token2), CurrencyAmount.fromRawAmount(token0, 10000), TradeType.EXACT_INPUT);
            });
        ...
        describe('tradeType = EXACT_OUTPUT', () => {
            let exactOut: Trade<Token, Token, TradeType.EXACT_OUTPUT>;
            beforeEach(async () => {
                exactOut = await Trade.fromRoute(new Route([pool_0_1, pool_1_2], token0, token2), CurrencyAmount.fromRawAmount(token2, 100n), TradeType.EXACT_OUTPUT);
            });
            ...
            it('returns slippage amount if nonzero', () => {
                expect(exactOut.minimumAmountOut(new Percent(0n, 100n)).equalTo(CurrencyAmount.fromRawAmount(token2, 100n))).toBeTruthy();
                expect(exactOut.minimumAmountOut(new Percent(5n, 100n)).equalTo(CurrencyAmount.fromRawAmount(token2, 100n))).toBeTruthy();
                expect(exactOut.minimumAmountOut(new Percent(200n, 100n)).equalTo(CurrencyAmount.fromRawAmount(token2, 100n))).toBeTruthy();
            });
        });
    });
    ...
    describe('#bestTradeExactOut', () => {
        it('throws with empty pools', async () => {
            await expect(Trade.bestTradeExactOut([], token0, CurrencyAmount.fromRawAmount(token2, 100n))).rejects.toThrow('POOLS');
        });
        it('throws with max hops of 0', async () => {
            await expect(
        ...
        it('respects maxHops', async () => {
            const result = await Trade.bestTradeExactOut([pool_0_1, pool_0_2, pool_1_2], token0, CurrencyAmount.fromRawAmount(token2, 10n), { maxHops: 1 });
            expect(result).toHaveLength(1);
            expect(result[0].swaps[0].route.pools).toHaveLength(1); // 0 -> 2 at 10:11
            expect(result[0].swaps[0].route.tokenPath).toEqual([token0, token2]);
        });
        ...
        it.skip('insufficient liquidity in one pool but not the other', () => {
            const result = Trade.bestTradeExactOut([pool_0_1, pool_0_2, pool_1_2], token0, CurrencyAmount.fromRawAmount(token2, 1050n));
            expect(result).toHaveLength(1);
        });

        it('respects n', async () => {
            const result = await Trade.bestTradeExactOut([pool_0_1, pool_0_2, pool_1_2], token0, CurrencyAmount.fromRawAmount(token2, 10n), { maxNumResults: 1 });

            expect(result).toHaveLength(1);
        });

        it('no path', async () => {
            const result = await Trade.bestTradeExactOut([pool_0_1, pool_0_3, pool_1_3], token0, CurrencyAmount.fromRawAmount(token2, 10n));
            expect(result).toHaveLength(0);
        });
        ...
        it('works for ETHER currency output', async () => {
            const result = await Trade.bestTradeExactOut([pool_weth_0, pool_0_1, pool_0_3, pool_1_3], token3, CurrencyAmount.fromRawAmount(Ether.onChain(250), 100n));
            expect(result).toHaveLength(2);
            expect(result[0].inputAmount.currency).toEqual(token3);
            expect(result[0].swaps[0].route.tokenPath).toEqual([token3, token0, WETH9[250]]);
            expect(result[0].outputAmount.currency).toEqual(ETHER);
