...
import { tickToPrice } from '../utils/price-tick-conversions';
import { SqrtPriceMath } from '../utils/sqrt-price-math';
    ...
    public get amount0(): CurrencyAmount<Token> {
        if (this._token0Amount === null) {
            this._token0Amount = CurrencyAmount.fromRawAmount(
                this.pool.token0,
                PositionMath.getToken0Amount(this.pool.tickCurrent, this.tickLower, this.tickUpper, this.pool.sqrtRatioX96, this.liquidity),
            );
        }
    ...
    public get amount1(): CurrencyAmount<Token> {
        if (this._token1Amount === null) {
            this._token1Amount = CurrencyAmount.fromRawAmount(
                this.pool.token1,
                PositionMath.getToken1Amount(this.pool.tickCurrent, this.tickLower, this.tickUpper, this.pool.sqrtRatioX96, this.liquidity),
            );
        }
