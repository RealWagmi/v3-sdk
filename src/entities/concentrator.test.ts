...
describe('Concentrator', () => {
    describe('constructor', () => {
        it('cannot be used for tokens on different chains', () => {
            expect(
                () => new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdc, '100'), CurrencyAmount.fromRawAmount(WETH9[ChainId.ZK_SYNC], '100')),
            ).toThrow('CHAIN_IDS');
        });
    });
    ...
    describe('#token0', () => {
        it('always is the token that sorts before', () => {
            expect(
                new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdc, '100'), CurrencyAmount.fromRawAmount(fantomTokens.usdt, '100')).token0,
            ).toEqual(fantomTokens.usdc);
            expect(
                new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdt, '100'), CurrencyAmount.fromRawAmount(fantomTokens.usdc, '100')).token0,
            ).toEqual(fantomTokens.usdc);
        });
    });
    describe('#token1', () => {
        it('always is the token that sorts after', () => {
            expect(
                new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdc, '100'), CurrencyAmount.fromRawAmount(fantomTokens.usdt, '100')).token1,
            ).toEqual(fantomTokens.usdt);
            expect(
                new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdt, '100'), CurrencyAmount.fromRawAmount(fantomTokens.usdc, '100')).token1,
            ).toEqual(fantomTokens.usdt);
        });
    });
    describe('#reserve0', () => {
        it('always comes from the token that sorts before', () => {
            expect(
                new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdc, '101'), CurrencyAmount.fromRawAmount(fantomTokens.usdt, '100')).reserve0,
    ...
    describe('#reserve1', () => {
        it('always comes from the token that sorts after', () => {
            expect(
                new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdc, '100'), CurrencyAmount.fromRawAmount(fantomTokens.usdt, '101')).reserve1,
            ).toEqual(CurrencyAmount.fromRawAmount(fantomTokens.usdt, '101'));
            expect(
                new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdt, '101'), CurrencyAmount.fromRawAmount(fantomTokens.usdc, '100')).reserve1,
            ).toEqual(CurrencyAmount.fromRawAmount(fantomTokens.usdt, '101'));
        });
    });
    ...
    describe('#chainId', () => {
        it('returns the token0 chainId', () => {
            expect(
                new Concentrator(zeroAddress, [], '100', CurrencyAmount.fromRawAmount(fantomTokens.usdc, '100'), CurrencyAmount.fromRawAmount(fantomTokens.usdt, '100')).chainId,
