...
import { BigintIsh, Currency, CurrencyAmount, Percent, TradeType, validateAndParseAddress } from '@real-wagmi/sdk';

import invariant from 'tiny-invariant';
import { swapRouterABI } from './abi/swap-router-abi';
import { Trade } from './entities/trade';
import { PermitOptions, SelfPermit } from './self-permit';
...
export abstract class SwapRouter {
     ...
     */
    public static swapCallParameters(trades: Trade<Currency, Currency, TradeType> | Trade<Currency, Currency, TradeType>[], options: SwapOptions): MethodParameters {
        ...
        const totalValue: CurrencyAmount<Currency> = inputIsNative ? trades.reduce((sum, trade) => sum.add(trade.maximumAmountIn(options.slippageTolerance)), ZERO_IN) : ZERO_IN;
