...
import { tickToPrice } from './price-tick-conversions';
import { priceToClosestTick } from './price-tick-conversions';

describe('priceTickConversions', () => {
    /**
     * Creates an example token with a specific sort order
     */
    function token({ sortOrder, decimals = 18, chainId = 1 }: { sortOrder: number; decimals?: number; chainId?: number }): Token {
        if (sortOrder > 9 || sortOrder % 1 !== 0) throw new Error('invalid sort order');
        return new Token(chainId, `0x${new Array<string>(40).fill(`${sortOrder}`).join('')}`, decimals, `T${sortOrder}`, `token${sortOrder}`);
    }
