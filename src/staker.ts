import { BigintIsh, Token, validateAndParseAddress } from '@real-wagmi/sdk';
import { Address, encodeAbiParameters, encodeFunctionData, Hex, parseAbiParameters } from 'viem';
import { v3StakerABI } from './abi/v3-staker-abi';
...
export abstract class Staker {
    ...
    private static INCENTIVE_KEY_ABI = 'tuple(address rewardToken, address pool, uint256 startTime, uint256 endTime, address refundee)';
    ...
    public static encodeDeposit(incentiveKeys: IncentiveKey | IncentiveKey[]): string {
        ...
        if (incentiveKeys.length > 1) {
            ...
            data = encodeAbiParameters(parseAbiParameters(Staker.INCENTIVE_KEY_ABI), [this._encodeIncentiveKey(incentiveKeys[0])]);
        }
        return data;
    }
