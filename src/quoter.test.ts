import { CurrencyAmount, Token, TradeType, WETH9 } from '@real-wagmi/sdk';
    ...
    describe('#swapCallParameters', () => {
        describe('single trade input', () => {
            ...
            it('multi-hop exact input', async () => {
                ...
                expect(calldata).toBe(
                    '',
                );
                expect(value).toBe('0x00');
            });
            ...
            it('multi-hop exact output', async () => {
                ...
                expect(calldata).toBe(
                    '',
                );
                expect(value).toBe('0x00');
            });
        ...
        describe('single trade input using Quoter V2', () => {
            ...
            it('single-hop exact input', async () => {
                const trade = await Trade.fromRoute(new Route([pool_0_1], token0, token1), CurrencyAmount.fromRawAmount(token0, 100), TradeType.EXACT_INPUT);
                const { calldata, value } = SwapQuoter.quoteCallParameters(trade.swaps[0].route, trade.inputAmount, trade.tradeType, { useQuoterV2: true });
